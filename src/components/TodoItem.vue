      ...
      <div class="mx-auto hidden group-hover:block">
        <button @click="removeTodo(todo.id)">
          <FontAwesomeIcon
            :icon="['far', 'trash-can']"
            size="6x"
            class="w-[100px] text-gray-100 transition-colors duration-300 hover:text-rose-100 active:text-rose-600"
          />
        </button>
      </div>

      <div
        class="mx-auto w-[445px] max-w-[489px] space-y-[12px] rounded-[8px] border-x-[1px] bg-white px-[33px] py-[26px] shadow-sm transition-all duration-300 group-hover:mx-[10px] group-hover:w-full group-hover:px-[55px]"
        :class="[
...
const isCompleted = computed(() => {
  if (props.todo.completed) {
    return 'text-green-600 bg-transparent hover:bg-slate-100 active:text-transparent hover:text-green-300'
  }
  return 'text-gray-100 hover:bg-transparent hover:text-green-300 active:text-green-600'
})
